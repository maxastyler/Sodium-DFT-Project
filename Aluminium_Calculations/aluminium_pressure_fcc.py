...
def get_pressure(lattice_param, ecutwfc):
    p=run(['pw.x'], stdout=PIPE, input=create_string(ecutwfc, lattice_param), encoding='ascii')
    print(ecutwfc)
    for line in p.stdout.splitlines():
        if "total   stress  (Ry/bohr**3)                   (kbar)     P=" in line:
            return float(line.split()[-1])
lat_params = np.linspace(7.55, 7.75, 40)
ecut_params = np.linspace(20, 40, 20)
pressures = [get_pressure(7.655, i) for i in ecut_params]
np.save("ecut_pressures_fcc", pressures)
...
plt.plot(ecut_params, pressures)
