...
for filename in sorted(filenames, key=filenames.__getitem__):
    with open("extracted/bcc.fqha."+str(filenames[filename])) as f:
        with open("extracted/bcc-li.scf.out."+str(filenames[filename])) as dft:
            for line in dft:
                if "!" in line:
...
for temp in tempdict:
    with open("bm_fits/"+"li.bcc."+str(temp), "w") as f:
