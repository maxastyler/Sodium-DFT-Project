    ...
    for f in listdir(join(folder, "pt_fits")):
        if fnmatch(f, "*.xml"):
            ...
            for line in tree.find("FIT_CHECK").find("VOL_ENE_EFIT_DELTA_P_GIBBS").text.splitlines():
                ...
                if split!=[]:
                    if i=="bcc":
                        pressures[float(split[4])/10.0]=float(split[5])-0.0001999997 
                    else:
...
print(bcc_lower)
con=plt.contourf(p_interp, t_interp, bcc_lower, 1)
plt.xlabel("Pressure (GPa)")
plt.ylabel("Temperature (K)")
plt.title("Sodium PT Diagram (Blue = FCC, Green = BCC)")
